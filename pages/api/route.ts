...
const GenAPI = process.env.NEXT_PUBLIC_API_GEMINI || 'YourAPIKeyHere'
...
async function runValidationAlgorithm(data: any): Promise<boolean> {
    ...
    async function validateWebsite(website: string): Promise<boolean> {
      try {
        // 
        const whoisApi = process.env.NEXT_PUBLIC_API_WHOIS || 'Your whoisxmplapi here'
        const whoisApiUrl = `https://www.whoisxmlapi.com/whoisserver/WhoisService?apiKey=${whoisApi}&domainName=${encodeURIComponent(website)}`;
